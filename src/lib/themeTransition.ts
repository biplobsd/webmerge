...
export function createThemeTransition({ isLightFun, toggleTheme }: ThemeTransitionOptions) {
  const toggleThemeWithAnimation = async (buttonElement: HTMLElement) => {
    ...
    try {
      ...
      const maxRadius = Math.hypot(Math.max(rect.left, window.innerWidth - rect.left), Math.max(rect.top, window.innerHeight - rect.top));
      ...
      document.documentElement.style.setProperty("--transition-z-index-new", isLight ? "998" : "999");
      document.documentElement.style.setProperty("--transition-z-index-old", isLight ? "999" : "998");
